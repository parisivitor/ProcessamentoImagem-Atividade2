...
/*-----------------------------------------------------------------
 * Esta funÃ§Ã£o calcula FFT com numeros complexo
 * x e y sÃ£o arrays de 2^m pontos de numeros reais e imaginarios, 
 * respectivamente:
 *   dir =  1 para calcular a transformaÃ§Ã£o direta
 *   dir = -1 para calcular a transformaÃ§Ã£o reversa
 * CÃ¡lculo:
 *   F_k = F_k^{par} + W^k F_k^{impar}
 *         onde: W^k = e^{2iPI/N}
 *   F_k^{ipippii...ipiip} = fn (termo trivial, calculado sem k)
 *   Se i (Ã­mpar) = 0 e p (par) = 1, 
 *      EntÃ£o ipippii...ipiip = 0101100...01001 
...
int FFT(int dir, int m, double *x, double *y) {
    ...
    /* Escala para a transformaÃ§Ã£o direta */
    if (dir == 1)
        for (i = 0; i < nn; i++) {
            x[i] /= (double) nn;
            y[i] /= (double) nn;
...
/*-----------------------------------------------------
 *  Calcula a potencia de 2 mais prÃ³ximo de um nÃºmero
 *  twopm = 2**m <= n
...
/*-------------------------------------------------------------------------
 * Executa um FFT 2D numa matriz de nÃºmeros complexos
 * A direÃ§Ã£o dir, 1 para direta, -1 para reversa
 * O tamanho da matriz (nl = numero de linhas, nc = numero de colunas)
 * Aborta com erro se nÃ£o tem memÃ³ria ou as dimensÃµes nÃ£o sÃ£o potÃªncia de 2.
 *-------------------------------------------------------------------------*/
void FFT2D(double _Complex *c, int nl, int nc, int dir) {
    ...
    if (real == NULL || imag == NULL)
        erro("alocaÃ§Ã£o de memÃ³ria - FFT2D");
    if (!Powerof2(nl, &m, &twopm) || twopm != nl)
        erro("Falhou no cÃ¡lculo das linhas - FFT2D");
    for (j = 0; j < nc; j++) {
        for (i = 0; i < nl; i++) {
    ...
    if (!Powerof2(nc, &m, &twopm) || twopm != nc)
        erro("Falhou no cÃ¡lculo das colunas - FFT2D");
    for (i = 0; i < nl; i++) {
        for (j = 0; j < nc; j++) {
...
void filtro(double _Complex *img, int nl, int nc) {
    ...
    for (i = 0; i < nl; i++)
        for (j = 0; j < nc; j++) {
            if ((j <= nc / 2 - nx || j >= nc / 2 + nx)){
                if((i <= nl / 2 - nx || i >= nl / 2 + nx)){
                    img[i * nc + j] = 0;
                }
            }
        }
...
/*-------------------------------------------------------------------------
 * ImplementaÃ§Ã£o da transformada rÃ¡pida de fourier
 * PARÃ‚METROS:
 *   In = imagem de entrada
 *   Out= imagem de saÃ­da
 *   nl = nÃºmero de linhas.
 *   nc = nÃºmero de colunas.
 *   mn = mÃ¡ximo nÃ­vel de cinza
 *-------------------------------------------------------------------------*/
